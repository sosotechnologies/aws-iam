...
locals {
  ...
  cluster_endpoint  = time_sleep.mimeo2025.triggers["cluster_endpoint"]
  cluster_name      = time_sleep.mimeo2025.triggers["cluster_name"]
  oidc_provider_arn = time_sleep.mimeo2025.triggers["oidc_provider_arn"]
...
data "aws_eks_addon_version" "mimeo2025" {
...
resource "aws_eks_addon" "mimeo2025" {
  for_each = var.eks_addons

  cluster_name = local.cluster_name
  addon_name   = try(each.value.name, each.key)

  addon_version               = try(each.value.addon_version, data.aws_eks_addon_version.mimeo2025[each.key].version)
...
resource "random_bytes" "mimeo2025" {
  length = 2
}
...
resource "aws_cloudformation_stack" "usage_telemetry" {
  ...
  name = "${var.cluster_name}-${random_bytes.mimeo2025.hex}"
